function setQuestion(qnum){
     $("#img1").attr("src",quiz.q[qnum].image);
     $("#img1").data("correct", quiz.q[qnum].answer);
      $("#b1").html(quiz.q[qnum].options[0]);
      $("#b2").html(quiz.q[qnum].options[1]);
      $("#b3").html(quiz.q[qnum].options[2]);
      $("#b4").html(quiz.q[qnum].options[3]);    
}
 
var count=15;
 
var counter=setInterval(timer, 1000); //1000 will  run it every 1 second
 
function timer()
{
  count=count-1;
  if (count <= 0)
  {
     clearInterval(counter);
     //counter ended, do something here
     return;
  }
$("#elapsedtime").html(count);
}
 
function resetTimer(totaltime){
    count = totaltime;
    if (count <= 0){
        clearInterval(counter);
    }
    var counter=setInterval(timer, 1000); //1000 will  run it every 1 second
 
}
$(document).ready(function() {
setQuestion(0);
    $("#q1").click(function() {
        $("#img1").attr("src", "675px-Estelada_blava.svg.png");
        $("#img1").data("correct", "Catalonia");
        $("#b1").html("North Cyprus");
        $("#b2").html("South Ossetia");
        $("#b3").html("Basque Country");
        $("#b4").html("Catalonia");
    });
 
    $("#q2").click(function() {
        $("#img1").attr("src", "Flag_of_Transnistria_(state).svg");
        $("#img1").data("correct", "Transnistria");
        $("#b1").html("Kurdistan");
        $("#b2").html("Chechnya");
        $("#b3").html("Transnistria");
        $("#b4").html("Nagorno-Karabakh");
    });
 
    $("#q3").click(function() {
        $("#img1").attr("src", "Chechen_flag.gif");
        $("#img1").data("correct", "Chechnya");
        $("#b1").html("Somaliland");
        $("#b2").html("Chechnya");
        $("#b3").html("Flanders");
        $("#b4").html("Andalusia");
    });
    }
    $(".ans").click(function() {
        buttonText = $(this).html();
        correctAnswer = $("#img1").data("correct");
        totalscore = $("#totalscore").text();
        if (buttonText === correctAnswer)
        {
            $("#totalscore").text(totalscore * 1 + 1);
            questnum = $("#questnum").text();
            //alert(questnum);
            $("#questnum").text(questnum*1 + 1);
            setQuestion (questnum);
   clearInterval(counter);
            
        }
        else {
            $("#totalscore").text(totalscore * 1 - 1);
        }
 
    });
 
});
 
var quiz = {
  "q": [
    {
      "image": "Flag_of_Transnistria_(state).svg",
      "answer": "Transnistria",
      "options": [
        "Abkhazia",
        "South Ossetia",
        "Kurdistan",
        "Transnistria"
      ]
    },
 
    {
      "image": "Chechen_flag.gif",
      "answer": "Chechnya",
      "options": [
        "Chechnya",
        "Somaliland",
        "Adjara",
        "Crimea"
      ]
    }
 
  ]
};

